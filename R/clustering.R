...
# distance: "euclidean", "manhattan", "spearman" or "pearson"
# clustMethod: "ward", "single", "complete", "average", "mcquitty",
#              "median" or "centroid"
# type: "samples" or "variables"
# num.clusters: number of clusters in kmeans

clustering <- function(dataset, method = "hc", distance = "euclidean",
                       type = "samples", num.clusters = 5,
  ...
  if (method == "hc") {
    ...
    stop("method must be either 'hc' or 'kmeans'", call. = FALSE)
  }
...
hierarchical_clustering <- function(dataset, distance = "euclidean",
                                    clustMethod = "complete",
                                    hc.type = "samples") {
  if (hc.type == "samples") {
...
kmeans_result_df <- function(kmeans.result) {
  ...
  for (i in seq_len(max(kmeans.result$cluster))) {
    kmeans.result.df <- rbind(
      kmeans.result.df,
      data.frame(
        cluster = i,
        samples = paste(
          names(kmeans.result$cluster[kmeans.result$cluster == i]),
          collapse = " "
        )
      )
    )
...
kmeans_plot <- function(dataset, kmeans.result) {
  ...
  for (i in seq_len(num.clusters)) {
    matplot(
      as.matrix(dataset$data[, kmeans.result$cluster == i, drop = FALSE]),
      type = "l",
      col = "grey",
      main = paste("Cluster ", i, ", n = ", kmeans.result$size[i], sep = ""),
      axes = FALSE
    )
...
# dendrogram
dendrogram_plot <- function(dataset, hc.result, column.metadata = 1,
                            labels = NULL, ...) {
  if (!requireNamespace("ggdendro", quietly = TRUE)) {
    stop(
      "Package 'ggdendro' needed for this function to work. Please install it with install.packages('ggdendro').",
      call. = FALSE
    )
  }
  ...
  if (!is.null(labels)) {
    labels.hc <- labels
  } else if (is.null(column.metadata)) {
...
dendrogram_plot_col <- function(dataset, hc.result, classes.col, colors = NULL,
                                title = "", lab.cex = 1.0,
                                leg.pos = "topright", label_samples = NULL, ...) {
  ...
  if (is.null(label_samples)) {
    ...
    label_samples <- dataset$metadata[, label_samples]
  }
  ...
  if (is.null(colors)) {
    leg.col <- seq_along(levels(classes))
  } else {
    leg.col <- colors
  }
